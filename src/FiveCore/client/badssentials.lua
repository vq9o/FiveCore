...
AddEventHandler('Badssentials:Announce', function(msg)
  exports['mythic_notify']:SendAlert('error', 'ANNOUNCEMENT: '.. msg)
	timer = 0;
	announcement = true;
	ann = msg;
	if #ann > 70 then
...
AddEventHandler('Badssentials:SetAOP', function(aop)
	currentAOP = aop;
  exports['mythic_notify']:SendAlert('error', 'Area Of Play is now '.. aop ..'. Move to the Area Of Play')
end)
RegisterNetEvent('Badssentials:SetPT')
AddEventHandler('Badssentials:SetPT', function(pt)
	peacetime = pt;
  if peacetime == true then
    exports['mythic_notify']:SendAlert('error', 'Peacetime is enabled, do not shoot or start prioritys')
  else
    exports['mythic_notify']:SendAlert('inform', 'Peacetime is over, do not immediately start a priority')
  end
end)
...
RegisterCommand("postal", function(source, args, raw)
	if #args > 0 then
      ...
      exports["mythic_notify"]:DoHudText("success", "Waypoint has been set to postal ".. args[1])
		else
      exports["mythic_notify"]:DoHudText("error", "Not a valid postal")
		end
	else
		SetWaypointOff();
		  exports["mythic_notify"]:DoHudText("inform", "Waypoint reset")
	end
end)
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        exports["mythic_notify"]:DoHudText("error", "Peacetime is enabled. You can not shoot.")
